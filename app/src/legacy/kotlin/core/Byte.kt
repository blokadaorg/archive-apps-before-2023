...
abstract class ByteVB(internal var onTap: (ByteView) -> Unit = {})
...
class ByteView(
    ...
    private val containerView = findViewById<ViewGroup>(R.id.byte_container)
    ...
    private val arrowView = findViewById<TextView>(R.id.byte_arrow)
    ...
    fun arrow(label: Resource?, color: Resource? = Resource.ofResId(R.color.colorAccent)) {
        when {
            ...
            label.hasResId() -> {
                ...
                arrowView.setText(label.getResId())
            }
            else -> {
                arrowView.visibility = View.VISIBLE
                arrowView.setText(label.getString())
            }
        }
