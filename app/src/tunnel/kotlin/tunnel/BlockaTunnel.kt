...
internal class BlockaTunnel(
    ...
    fun openGatewaySocket(ktx: Kontext) {
        ...
        proxy.forward = { ktx, bufferId ->
            forwardQueued = true
            forwardBuffers[currentForwardBuffer] = buffers[bufferId]
            forwardBuffersReverse[currentForwardBuffer] = bufferId
            lastForwardBuffer = currentForwardBuffer++
        }
    ...
    override fun run(ktx: Kontext, tunnel: FileDescriptor) {
        ...
        try {
            ...
            while (true) {
                ...
                fromProxyToGateway(ktx, gateway)
    ...
    private fun fromProxyToGateway(ktx: Kontext, gateway: StructPollfd) {
        if (gateway.isEvent(OsConstants.POLLOUT)) {
            repeat(lastForwardBuffer + 1) {
                val b = forwardBuffers[it]!!
                val id = forwardBuffersReverse[it]!!
//                ktx.v("XXX toGateway, $id ${b.arrayOffset()} ${b.position()} ${b.limit()}")
