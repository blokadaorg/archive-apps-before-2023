...
internal class BlockaProxy(
    ...
    override fun fromDevice(ktx: Kontext, fromDevice: ByteArray, length: Int) {
        ...
        do {
            ...
            when (op[0].toInt()) {
                BoringTunJNI.WRITE_TO_NETWORK -> forward(ktx, destinationId)
                BoringTunJNI.WIREGUARD_ERROR -> {
                    ktx.e("wireguard error: $response")
                    buffers.returnBuffer(destinationId)
                }
                BoringTunJNI.WIREGUARD_DONE -> {
                    if (i == 1) ktx.e("read: did not do anything with packet")
                    buffers.returnBuffer(destinationId)
                }
                else -> {
    ...
    override fun toDevice(ktx: Kontext, source: ByteArray, length: Int, originEnvelope: Packet?) {
        ...
        do {
            ...
            when (op[0].toInt()) {
                BoringTunJNI.WRITE_TO_NETWORK -> forward(ktx, destinationBufferId)
                BoringTunJNI.WIREGUARD_ERROR -> {
                    ktx.e("read: wireguard error: $response, length: $length")
    ...
    fun tick(ktx: Kontext) {
        ...
        when (op[0].toInt()) {
            ...
            BoringTunJNI.WIREGUARD_ERROR -> {
                buffers.returnBuffer(destinationBufferId)
                ktx.e("tick: wireguard error: $response")
            }
            BoringTunJNI.WIREGUARD_DONE -> {
                buffers.returnBuffer(destinationBufferId)
            }
